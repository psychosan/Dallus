...
namespace Dallus
{
    ...
    public class Repox : IRepox 
    {
        ...
        public bool DeleteWhere<T>(Expression<Func<T, bool>> predicate) where T : class, IRepoModel
        {
            var sql = new SqlLam<T>(predicate);
        ...
        /// <returns></returns>
        public T GetById<T>(dynamic pkVal) where T : class, IRepoModel
        {
        ...
        /// <param name="pkVal"></param>
        /// <returns></returns>
        public T FindById<T>(dynamic pkVal) where T : class, IRepoModel
        {
            string selectByIdScript = GetModelDetail<T>(ModelDetail.SelectById);
            return WithConnection<T>(k => k.QuerySingle<T>(selectByIdScript, new { pkVal }, commandType: CommandType.Text));
        }

        public T GetSingle<T>(dynamic pkId) where T : class, IRepoModel
        {
            string selectByIdScript = GetModelDetail<T>(ModelDetail.SelectById);
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public T GetSingleWhere<T>(Expression<Func<T, bool>> predicate) where T : class, IRepoModel
        {
            var sql = new SqlLam<T>(predicate);
            string whereCond = LambdaExtractSqlWhere(sql);
            string qryScript = GetModelDetail<T>(ModelDetail.SelectWhereScript);
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public IEnumerable<T> GetPage<T>(int page = 1, int pageSize = 15) where T : class, IRepoModel
        {
            page = page < 1 ? 1 : page;
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<T> GetList<T>() where T : class, IRepoModel
        {
        ...
        /// <param name="whereCondition"></param>
        /// <returns></returns>
        public IEnumerable<T> GetListWhere<T>(string whereCondition) where T : class, IRepoModel
        {
        ...
        /// <param name="predicate"></param>
        /// <returns></returns>
        public IEnumerable<T> GetListWhere<T>(Expression<Func<T, bool>> predicate) where T : class, IRepoModel
        {
            var sql = new SqlLam<T>(predicate);
            string whereCondition = LambdaExtractSqlWhere(sql);
        ...
        public int Count<T>() where T : class, IRepoModel
        {
            try
            {
        ...
        public int CountWhere<T>(string whereCondition, object parameters = null) where T : class, IRepoModel
        {
            try
            {
                whereCondition = CleanWhereClause(whereCondition);
        ...
        public int CountWhere<T>(Expression<Func<T, bool>> predicate) where T : class, IRepoModel
        {
            var sql = new SqlLam<T>(predicate);
            string whereCondition = LambdaExtractSqlWhere(sql);
            string qryScript = GetModelDetail<T>(ModelDetail.CountWhereScript);
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public IEnumerable<T> ExecSproc<T>(string sprocName, object? parameters = null) where T : class, IRepoModel
        {
            return WithConnection<IEnumerable<T>>(k => k.Query<T>(sprocName, parameters));
        }

        public T ExecSprocSingle<T>(string sprocName, object? parameters = null) where T : class, IRepoModel
        {
            return WithConnection<T>(k => k.QuerySingle<T>(sprocName, parameters));
        }

        public bool ExecAction<T>(string sqlScript, object? parameters = null, CommandType commandType = CommandType.Text) where T : class, IRepoModel
        {
            return WithConnection<bool>(k => k.Execute(sqlScript, parameters, commandType: commandType) > 0);
        }

        public IEnumerable<T> ExecAny<T>(string sqlScript, object? parameters = null, CommandType commandType = CommandType.Text) where T : class, IRepoModel
        {
            return WithConnection<IEnumerable<T>>(k => k.Query<T>(sqlScript, parameters, commandType: commandType));
        }

        public T ExecAnySingle<T>(string sqlScript, object? parameters = null, CommandType commandType = CommandType.Text) //This can be anything.. so no => where T : class, IModel
        {
        ...
        // Summary: Execute parameterized SQL
        // Returns: Number of rows affected
        public int Execute(string sql, object? param = null, IDbTransaction? transaction = null, int? commandTimeout = default, CommandType? commandType = default)
        {
            try
            {
        ...
        public dynamic QueryFirst(string sql, object param = null, IDbTransaction transaction = null,
            int? commandTimeout = default, CommandType? commandType = default)
        {
            try
            {
        ...
        internal T WithConnection<T>(Func<IDbConnection, T> getData)
        {
            try
            {
        ...
        internal async Task<T> WithConnectionAsync<T>(Func<IDbConnection, Task<T>> getData)
        {
            try
            {
                using (var connection = new SqlConnection(_connection))
        ...
        internal async Task WithConnectionAsync(Func<IDbConnection, Task> getData)
        {
            try
            {
                using (var connection = new SqlConnection(_connection))
        ...
        internal void RefreshCacheItem<T>() where T : class, IRepoModel
        {
        ...
        private string LambdaExtractSqlWhere<T>(SqlLam<T> sqlamda) where T : class, IRepoModel
        {
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        private ModelInfo TryGetCacheItem<T>() where T : class, IRepoModel
        {
        ...
        private string CleanWhereClause(string whereCondition)
        {
        ...
        /// <returns></returns>
        public dynamic SimplePkCast(dynamic objValue, bool isNumeric)
        {
            if (isNumeric)
        ...
        public bool NotNullEmptyOrZero(dynamic value)
        {
